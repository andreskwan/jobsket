...
@implementation FavoritesDetailTVC


@synthesize navController, favoritesDetailMapVC, tableView, jobMo;
...
- (void)tapped:(UITapGestureRecognizer *)gesture {
	...
	debug(@"set %@ hidden to %@", self.uiTabBarController, !barsHidden ? @"YES" : @"NO");
}
...
- (void)viewWillAppear:(BOOL)animated {
	...
	UIImage *backImage = [[Themes getCachedImage:PNG_TOP_BAR_FAVORITES] stretchableImageWithLeftCapWidth:10 topCapHeight:10];
...
- (void)viewDidLoad {
	...
	parentWindow = ((FavoritesUINavigationController*) self.navigationController).aWindow;
	
	// nytimes bars
//	UIBarButtonItem *item = [[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAction target:self action:@selector(share)] autorelease];
...
- (void) touchUpInsideSegmentIndex:(int)index {
    debug(@"touchUpInsideSegmentIndex %d", index);
    switch (index) {
    	...
    	case 1: // map
			...
			favoritesDetailMapVC.jobs = [NSSet setWithObject:jobMo];
			[navController pushViewController:favoritesDetailMapVC animated:YES];
            debug(@"navController=%@, favoritesDetailMapVC=%@",navController,favoritesDetailMapVC);
	    	break;
